...
'Researcher': { icon: '🔬', showComponents: true, sections: [
...
'Health Worker': { icon: '🏥', showComponents: true, sections: [
...
'Resident': { icon: '🏘️', showComponents: false, sections: [
...
'Urban Planner': { icon: '🏙️', showComponents: true, sections: [
...
'Wildlife Responder': { icon: '🐍', showComponents: true, sections: [
...
'Farmer': { icon: '🌾', showComponents: false, sections: [
...
'Journalist': { icon: '📰', showComponents: true, sections: [
...
'Student': { icon: '📚', showComponents: true, sections: [
...
'Tour Guide': { icon: '🗺️', showComponents: false, sections: [
    { id: 'verdict', icon: '🗺️', title: 'Is This Area Safe to Visit?', subtitle: 'Safety verdict for your tour', type: 'verdict' },
    { id: 'sightings', icon: '📍', title: 'Active Sightings Zones', subtitle: 'Where snakes have been spotted', type: 'sightings' },
...
'First Responder': { icon: '🚨', showComponents: false, sections: [
...
function fillSectionBody(body, type, data, sightings, occurrences, allResults) {
    ...
    switch (type) {
        case 'sightings': renderCardList(body, sightings.length > 0 ? sightings : allResults, CARDS_PER_PAGE); break;
        case 'occurrences': renderCardList(body, occurrences, CARDS_PER_PAGE); break;
        case 'venomous': {
            const ven = allResults.filter(r => VENOMOUS_TERMS.some(t => (r.common_name || r.species || '').toLowerCase().includes(t)));
            if (ven.length === 0) { body.innerHTML = `<p class="no-results">No confirmed venomous species detected in results.</p>`; } 
            else { renderCardList(body, ven, CARDS_PER_PAGE); }
            break;
        }
        case 'species': {
        ...
        case 'headline': {
            const venCount = allResults.filter(r => VENOMOUS_TERMS.some(t => (r.common_name || r.species || '').toLowerCase().includes(t))).length;
            body.innerHTML = `
                 <div class="info-row">
        ...
        case 'emergency': {
            const slug = (risk.label || 'unknown').toLowerCase();
            const venomous = allResults.filter(r => VENOMOUS_TERMS.some(t => (r.common_name || r.species || '').toLowerCase().includes(t)));
            body.innerHTML = `
                 <div class="verdict-block ${slug}">
        ...
        default: body.innerHTML = `<p class="no-results">No data available.</p>`;
    }
}

function renderCardList(container, results, limit) {
    if (!results || results.length === 0) { container.innerHTML = `<p class="no-results">No records found.</p>`; return; }
    ...
    visible.forEach((item, index) => {
        ...
        const scientificLine = item.species && item.common_name && item.species !== item.common_name ? `<span class="card-scientific">${item.species}</span>` : '';
        ...
        card.addEventListener('click', () => {
            if (item.latitude && item.longitude) {
                if (!mapOpen) { mapOpen = true; resultsBody.classList.add('map-open'); initMap(); }
                setTimeout(() => { if (map) { map.invalidateSize(); map.flyTo([item.latitude, item.longitude], 12, { animate: true, duration: 1 }); } }, 400);
            }
        });
    ...
    if (remaining > 0) {
        ...
        btn.addEventListener('click', () => { btn.remove(); renderCardList(container, results.slice(limit), results.length); });
...
function renderRiskScore(risk) {
    ...
    if (config.showComponents && risk.components) {
        Object.values(risk.components).forEach(comp => {
            ...
            el.innerHTML = `<span class="rc-label">${comp.label}</span><div class="rc-bar-wrap"><div class="rc-bar" style="width:${pct}%"></div></div><span class="rc-score">${comp.score} / ${comp.max}</span>`;
    ...
    btn.addEventListener('click', () => {
        ...
        if (layer === 'habitat') {
            if (active) {
                ...
                if (ee && ee.tile_url) { habitatLayer = L.tileLayer(ee.tile_url, { opacity: 0.7, attribution: 'GEE Hansen Forest Loss' }); habitatLayer.addTo(map); } 
                else { btn.classList.remove('active'); }
            } else { if (habitatLayer) { map.removeLayer(habitatLayer); habitatLayer = null; } }
        }
        if (layer === 'urban') {
            const cop = currentSearchData && currentSearchData.copernicus;
            if (active) {
                if (cop && cop.wms_url && cop.wms_layer) {
                    urbanLayer = L.tileLayer.wms(cop.wms_url, { layers: cop.wms_layer, format: 'image/png', transparent: true, opacity: 0.6, attribution: 'Copernicus Urban Atlas', version: '1.3.0' });
                    urbanLayer.addTo(map);
                } else { btn.classList.remove('active'); }
            } else { if (urbanLayer) { map.removeLayer(urbanLayer); urbanLayer = null; } }
        }
        if (layer === 'sightings') { if (active) markersLayer.addTo(map); else map.removeLayer(markersLayer); }
    });
});